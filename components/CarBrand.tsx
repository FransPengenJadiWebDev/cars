  ...
  return (
    <div className='w-full mb-15 lg:mb-25 mt-10'>
      <div className='mb-5 flex flex-row justify-between items-center'>
          <span className='text-white text-[18px] lg:text-[25px] font-semibold tracking-wider uppercase'>Premium Brands</span>
          <Link 
            href="/brands" 
            className='hidden lg:flex flex-row gap-2 items-center text-white/50 hover:text-orange-500 transition-colors duration-300 text-sm tracking-wider'
          >
      ...
      <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6'>
          {displayedBrand.map((brand, index) => {
            ...
            return (
              <button 
                key={index} 
                onClick={() => onSelectedBrand(isSelected ? null : brand.car_brand)} 
                className={`group flex flex-col items-center justify-center h-32 w-full border ${isSelected ? 'border-orange-500/50': 'bg-neutral-900 border-neutral-800 hover:border-orange-500/50'} rounded-xl transition-all duration-300 cursor-pointer`}
              >
                ...
                <span className={`mt-2 text-[10px] uppercase tracking-[0.2em] ${isSelected ? 'text-white' : 'text-neutral-500 group-hover:text-white'} transition-colors`}>
                  {brand.car_brand}
                </span>
              </button>
            )
