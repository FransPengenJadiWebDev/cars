...
const NewsCard = ({post, layout} : Props) => {
  ...
  return (
    <Link 
        href='/news'
        key={post.id} 
        className={`lg:h-[400px] ${layout} group flex ${layout === 'flex-col' ? 'h-[350px]' : 'h-full'} bg-neutral-950 border border-neutral-900 rounded-xl overflow-hidden hover:border-orange-500/20 transition-all duration-300`}
    >
        <div className={`${layout === 'flex-row' ? 'flex-3 lg:flex-1' : 'flex-1'} relative lg:w-full  bg-neutral-900`}>
            <Image src={post.image} alt={post.title} fill className='object-cover group-hover:scale-105 transition-all duration-700'/>
        </div>
        <div className={`${layout === 'flex-row' ? 'flex-2 lg:flex-1' : 'flex-1'} p-4 lg:p-5 flex flex-col gap-2 lg:gap-3`}>
